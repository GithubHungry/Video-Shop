...
public static function getOrderByIdOne($id)
    {
          // Соединение с БД
        $db = Db::getConnection();
...
//        }
    ...
    public static function getOrdersListUs($user_id)
    {
        // Соединение с БД
        $db = Db::getConnection();

        // Получение и возврат результатов
        $result = $db->query('SELECT * FROM product_order WHERE user_id = '.$user_id);
        $ordersList = array();
        $i = 0;
        while ($row = $result->fetch()) {
            $ordersList[$i]['id'] = $row['id'];
